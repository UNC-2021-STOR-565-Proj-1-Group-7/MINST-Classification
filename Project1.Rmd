...
# Abstract
Our report studied the MNIST handwritten digits data by conducting exploratory data analysis, principal component analysis, unsupervised K-means clustering method, and supervised K-Nearest Neighbor classification algorithm. We firstly provided statistical summary of the MNIST handwritten digits data to have an overview of data information. Since our data contained high dimensions, we utilized principal component analysis for dimension reduction. Then we used both supervised and unsupervised methods to classify the images of handwritten digits. We applied k-means algorithm on dimension-reduced data to categorize the images in our data into 9 clusters. The KNN classification algorithm provides a better classification model for our data with more than 90% accuracy on both training and testing set. 

# Introduction

We used the MNIST handwritten digits dataset from http://yann.lecun.com/exdb/mnist/, which is a collection of grey-scale images of handwritten digits that contains 1 categorical variable indicating the label of digits (from 0-9) and 784 numerical variable representing the pixels of the images. Since some of the handwritten digits are curly and blur which may create difficulties for computers to classify, it is interesting to explore how well different machine learning algorithms may perform for classification of these handwritten digits images. The models we trained might be suitable for other future similar gary-scale image classification. 
The MNIST has quite high dimension, and we were curious whether the high dimensionality will create any problem for classification. [NEED REVISION]
...
```{r, eval = FALSE}
k <- 9
minst_kmeans_df = minst_pcs$x[, 1:11]
k_cluster <- kmeans(minst_kmeans_df, centers = k, nstart = 25, iter.max = 100)

# plots to compare
fviz_cluster(k_cluster, geom = "point", data = minst_kmeans_df) + ggtitle("KMeans Clusters")
#k10
```
...
minst_pcs_cluster_df <- cbind(minst_sample.together[, c(1)], k_cluster$cluster)
minst_pcs_cluster_df <- data.frame(minst_pcs_cluster_df)
minst_pcs_cluster_df[,1] <- factor(minst_pcs_cluster_df[,1])
minst_pcs_cluster_df[,2] <- factor(minst_pcs_cluster_df[,2])
colnames(minst_pcs_cluster_df)[1] <- "digit"
colnames(minst_pcs_cluster_df)[2] <- "cluster"

ggplot(data = minst_pcs_cluster_df, aes(fill = digit)) + aes(x = cluster) + geom_bar()
```
...
withinclustersos <- data.frame(k_cluster$withinss)
for(i in 1:k){
  row.names(withinclustersos)[i] <- paste0("Cluster", i) 
}

compared_to_total <- withinclustersos[, 1] / k_cluster$tot.withinss
withinclustersos <- cbind(withinclustersos, compared_to_total)
withinclustersos <- rbind(withinclustersos, c(k_cluster$tot.withinss, 1))
rownames(withinclustersos)[k + 1] <- "Total"
colnames(withinclustersos) <- c("Within.Cluster", "Compared.Total")
withinclustersos[order(withinclustersos$Compared.Total), ]
```

The within-cluster sum of squares of all clusters, and their ratio to the total sum of squares are shown in the table. We observed that the clusters 5, 6, 7, 9 have low sum of squares. The clusters 10, 4, and 8 have medium sum of squares, and the clusters 2, 1, and 3 have high sum of squares. The magnitude of the sum of squares roughly correspond to the 
...
minst_pcs_pair_df <- cbind(minst_pcs_pair_df, minst_pcs_df[minst_pcs_df$PC == "PC3", c(3)])
colnames(minst_pcs_pair_df)[4] <- "PC3"
minst_pcs_cluster_pair_df <- cbind(minst_pcs_pair_df, factor(k10$cluster))
...
We use KNN Classification method with k values 5 on the training set. We calcualted the accuracy of the fitted model on the training set as 96. 
```{r, eval = FALSE}
...
minst_train_predict <- knn(minst_classification_train_pcs$x[, 1:pcs_classify],minst_classification_train_pcs$x[, 1:pcs_classify],cl=minst_target_category,k=5)

# compute accuracy on training set
tab_train <- table(minst_train_predict,minst_target_category)
...
We the run the fitted model on the test set and get the accuracy value 92.9. It's a bit lower compared to the training set, but the accuracy is still quite high. Since our model is trained based on the training set, it intuitively makes sense that the test set might have lower accuracy compared to the training set. 
```{r, eval = FALSE}
...
minst_test_predict <- knn(minst_classification_train_pcs$x[, 1:pcs_classify],minst_classification_test_pcs[, 1:pcs_classify],cl=minst_target_category,k=5)
# Compute accuracy on test set.
tab <- table(minst_test_predict,minst_test_category)
accuracy <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}
accuracy(tab)
```
...
cm <- confusionMatrix(factor(minst_test_predict), factor(minst_test_category), dnn = c("Prediction", "Label"))

ggplot(as.data.frame(cm$table), aes(Prediction,sort(Label,decreasing = T), fill= Freq)) +
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#009194") +
        labs(x = "Label",y = "Prediction") +
        scale_x_discrete(labels=c("Class_0", "Class_1","Class_2","Class_3","Class_4", "Class_5", "Class_6", "Class_7", "Class_8", "Class_9")) +
        scale_y_discrete(labels=c("Class_9", "Class_8", "Class_7", "Class_6", "Class_5", "Class_4","Class_3","Class_2","Class_1", "Class_0"))
```
According to the matrix, our algorithm predicts class1 and class7 much better than others, and it predicts class 2, 8, 9 much worse than others. Such condition makes sense because by nature 8 and 9 might be easily get confused since they are both round, while 1 and 7 are much easier to tell because 1 is straight and 7 has clearer edges. 
