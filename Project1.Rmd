...
split_minst <- function (minst, digit, train_amount = 2400, test_amount = 600) {
...
Homogeneity means all of the observations with the same class label are in the same cluster. The cluster 7 and 9 are relatively homogeneous as their standard deviation for labels are only around 1, all other clusters have higher standard deviations are are relatively heterogeneous. 
...
The within-cluster sum of squares of all clusters are shown in the table. We observed that only cluster 3 has much less sum of squares compared to the total sum of squares, all other clusters have similar or more sum of squares than the total sum of squares. 
```{r, eval = FALSE}
...
```{r}
head(minst_pcs_pair_df)
head(k10$cluster)
cbind(minst_pcs_pair_df, k10$cluster)
```
...
```{r}
summary(minst_pcs_cluster_pair_df$cluster)
```
...
For our dataset, we only have numerical variables since they are pixel values at different positions. [NEED REVISION]
